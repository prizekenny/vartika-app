  ...
  const tokenResponse = await axios.post(
    ...
    null,
    {
      params: {
        ...
        redirect_uri: `${process.env.BASE_URL}/auth/gmail/callback`,
...
router.get("/callback", async (req, res) => {
  try {
    ...
    if (!refresh_token) {
      console.log(`⚠️ No refresh_token received for Gmail: ${email}`);
      return res.status(400).json({ error: "No refresh_token received" });
    }

    await updateToken(email, "gmail", null, refresh_token, null, null);

    console.log(`✅ Stored refresh_token for Gmail: ${email}`);
...
router.get("/status", (req, res) => {
  const recentOAuthSuccess = gmailOAuthSuccess;
